...
async def handle_topic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            await update.message.reply_document(
                ...
                caption=f"📦 **Complete Learning Package**\n\n"
                       ...
                       f"• {vocab_filename} (Anki collocations file)\n"
        ...
        # Step 5: Generate and send reaction texts with audio
        await update_progress(5, "🎤 Generating reaction texts with audio...")
        
        # Voice mapping for different reactions
        voice_mapping = {
            'positive': 'en-US-Chirp3-HD-Achernar',
            'negative': 'en-GB-Chirp3-HD-Puck',
            'mixed': 'en-US-Chirp3-HD-Despina'
        }
        
        perspectives = [
            ("positive", "Positive Reaction", "😊", voice_mapping['positive']),
            ("negative", "Critical Reaction", "🤔", voice_mapping['negative']),
            ("mixed", "Balanced Reaction", "⚖️", voice_mapping['mixed'])
        ]
        
        # Generate all reaction audio concurrently
        reaction_tasks = []
        for key, name, emoji, voice in perspectives:
            reaction_tasks.append(generate_tts_async(content['opinion_texts'][key], voice_name=voice))
        
        reaction_audios = await asyncio.gather(*reaction_tasks, return_exceptions=True)
        
        for (key, name, emoji, voice), audio_data in zip(perspectives, reaction_audios):
            reaction_text = content['opinion_texts'][key]
            
            # Send text
            await update.message.reply_text(f"{emoji} **{name}:**\n\n{reaction_text}", parse_mode='Markdown')
...
def main():
    ...
    print(f"- Content level: CEFR B2 / weak C1 (200-250 word main text)")
    print(f"- Focus: Phrasal verbs with objects + useful collocations")
    print(f"- Voices: Achernar (positive), Puck (negative), Despina (mixed)")
